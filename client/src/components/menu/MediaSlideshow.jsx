...
function MediaSlideshow({ media = [], compact = false, alt = '' }) {
  ...
  return <div className="group relative h-full w-full overflow-hidden bg-[var(--color-surface)]">
    {current.resourceType === 'video' ? <video src={current.url} className="h-full w-full object-contain transition-transform duration-700 ease-out group-hover:scale-[1.045]" muted loop autoPlay playsInline /> : <img src={current.url} alt={current.altText || alt} className="h-full w-full object-contain transition-transform duration-700 ease-out group-hover:scale-[1.045]" />}
    {media.length > 1 && <>
