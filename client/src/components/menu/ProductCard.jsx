...
function ProductCard({ product }) {
  ...
  return <article className="group overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[var(--shadow-soft)] bakery-card">
    <Link to={`/menu/${product.id}`} className="bakery-media-frame relative block aspect-[4/3] overflow-hidden border-b border-[var(--color-border)]"><MediaSlideshow media={product.media} alt={product.name} compact /><OfferBadge product={product} className="absolute left-4 top-4" /></Link>
